...
function createShouldClauses({ parsedText, formatted }: CreateShouldClauses) {
  ...
  functions.push({
    script_score: {
      script: {
        ...
        params: {
          venueName:
            venueName.toLowerCase() || parsedText?.address?.toLowerCase() || "",
        },
      },
    },
  })
...
async function createSearchBody({
  ...
  // create query
  let query = createQuery({ parsedText, formatted })
